...
def run_simulation(A, B, C, STABILITY, half, useRules: bool = True):
    RULES = get_rules(half, useRules=useRules)
    return simulate(A, B, C, STABILITY, RULES, time_limit=get_time_limit(half=half))

def simulate_all(initial_stability, half: int, useRules: bool) -> list[SimulationResult]:
    ...
    for A in range(lower, upper + 1, iterator):
        ...
        for B in range(lower, upper + 1, iterator):
            for C in range(lower, upper + 1, iterator):
                results.append(simulate(A=A, B=B, C=C, STABILITY=initial_stability, rules=rules, time_limit=get_time_limit(half)))
